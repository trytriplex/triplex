...
While viewing a `tsx` or `jsx` file selecting the <ActionLink name="Triplex: Open File" href="#" /> action in the [Command Palette](https://code.visualstudio.com/docs/getstarted/userinterface#_command-palette) will open the first found component in that file in Triplex.
...
If there's a problem and the editor has been put into an invalid state you can reload the editor by accessing the command palette via <Kbd>Shift</Kbd> + <Kbd>Cmd</Kbd> + <Kbd>P</Kbd> and selecting "Developer: Reload Webviews".
